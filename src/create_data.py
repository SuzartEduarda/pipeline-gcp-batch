...
def obter_corte_data(is_incremental: Optional[bool] = None, delta_days: Optional[int] = None) -> datetime:
    ...
    if not inc:
        logging.info(f"[MODO BACKFILL] Gerando dados fake historicos a partir de {DATA_MINIMA}")
        return DATA_MINIMA

    calculo_decorte = datetime.now() - timedelta(days=days)
    corte = max(DATA_MINIMA, calculo_decorte)
    logging.info(f"[MODO INCREMENTAL] Janela de {days} dia(s) -> Data Limite de corte: {corte.strftime('%Y-%m-%d %H:%M:%S')}")
    return corte
...
# Função geradora de dados mockadosdinamicamente, via Lib Faker
def criar_dados(total_registros: int = 1000) -> List[Dict]:
    ...
    dias_totais = max(1, (data_fim - data_inicio).days)
    ...
    for idx in range(1, total_registros + 1):
        ...
        # Calculo de datas aleatorias
        dias_rand = random.randint(0, dias_totais)
        horas_rand = random.randint(7, 21)
        minutos_rand = random.randint(0, 59)
        dt_postagem = data_inicio + timedelta(days=dias_rand, hours=horas_rand, minutes=minutos_rand)

        registro = {
            "id_reclamacao": f"CG-MOCK-2026-{idx:05d}",
...
def dados_reclamacao(is_incremental: Optional[bool] = None, delta_days: Optional[int] = None) -> Tuple[List[Dict], bool]:
    ...
    # Simular conexão com fonte
    simular_conexao()

    # Gerar dados enriquecidos via Faker
    todos_registros = criar_dados(total_registros=5000)

    reclamacoes_filtradas = []
    has_error = False

    try:
        # Aplicar os filtros temporais sobre os dados gerados
        for rec in todos_registros:
            dt_post = datetime.strptime(rec["data_postagem"], "%Y-%m-%d %H:%M:%S")
            if dt_post >= corte_dedata:
                reclamacoes_filtradas.append(rec)

        logging.info(f"Geração Dinâmica via Faker Concluida. Total de {len(reclamacoes_filtradas)} reclamações OK")
    except Exception as e:
        logging.error(f"Erro durante o processamento de dados sinteticos com Faker: {str(e)}")
        has_error = True
    finally:
        # Simula o encecrramento da conexão
        simula_desconexao()

    return reclamacoes_filtradas, has_error
